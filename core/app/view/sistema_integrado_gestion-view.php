...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			...
			<div class="box">
		  		<div class="box-header">
		  			<h4>Cronograma de Capacitaciones</h4>
		  		</div>
		  		<div class="box-body">
		  		    <div class="col-md-12">
	  					<label>Año</label>
		  			...
		  			<table class="table table-hover table-bordered table-striped" id="tabla_cronograma">
		  				<thead>
	...
	<div class="modal-dialog" role="document" style="width: 45%;">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h3 class="modal-title" id="exampleModalLabel">Cambiar Estado Capacitación</h3>
			</div>
...
<script type="text/javascript">
	...
	function add_elemento_to_form() {
		aux_form++;
		$("#div_add").append(`
			<div class="col-md-5 form_${aux_form}" style="margin-top: 0.5rem;">
				<label>Mes</label>
				<select name="mes[]" id="mes" class="form-control">
					<option value="">--SELECCIONA--</option>
					<option value="0">Enero</option>
			...
			<div class="col-md-2 form_${aux_form}" style="text-align: center; margin-top: 0.5rem;">
	...
	function hecho(id){
		$.post('core/app/view/colaborador.php?parAccion=hecho', {id: id}, function(data) {
	...
	function no_hecho(id){
		$.post('core/app/view/colaborador.php?parAccion=no_hecho', {id: id}, function(data) {
			var obj = JSON.parse(data);
			if(obj.Result == "OK"){
				bootbox.alert({
					message: '<div class="alert alert-success fade in" style="margin-top: 5%; margin-bottom: 0;">'+
	...
	function get_cronograma(){
        ...
        $("#btn_pdf_cronograma").attr("href", "core/app/view/pdf-cronograma.php?anio="+$("#anio").val());
		...
		$.post('core/app/view/colaborador.php?parAccion=get_cronograma', {
			...
			$.each(obj, function(index, val) {
				...
				for (var i = 0; i < meses.length; i++) {
					cuerpo += `<td id="td_${i}_${val.id}" style="text-align: center; vertical-align: middle;" title="Clic para cambiar estado" data-toggle="modal" data-target="#modal_cambiar_estado">
					</td>`;
				}
				...
				$.each(val.fechas, function(index, va) {
					...
					$("#td_"+va.mes+"_"+val.id).append(`<strong style="font-weight: 900; font-size: 20px;">${va.dia.padStart(2, "0")}</strong>`);
				 ...
				 }); /*(var j = 0; j < meses_.length; j++) {
					
				}*/
				/*for (var j = 0; j < meses_.length; j++) {
					meses_valor[meses_[j]] = meses_valor[meses_[j]] + 1;
					programado += 1;
					if(val.estado == 1){
						ejecutado += 1;
						meses_aux[meses_[j]] = meses_aux[meses_[j]] + 1;
					}
					$("#td_"+meses_[j]+"_"+val.id).append(`<strong style="font-weight: 900; font-size: 20px;">${el_dia.padStart(2, "0")}</strong>`);
					$("#td_"+meses_[j]+"_"+val.id).addClass(cl);
					$("#td_"+meses_[j]+"_"+val.id).attr('onclick', 'preparar_cambio_estado('+val.id+', '+val.estado+');');
					$("#td_"+meses_[j]+"_"+val.id).css('cursor', 'pointer');
				}*/
				
				$("#td_anuo_"+val.id).append(((ejecutado*100)/programado)+"%");
			});
			for (var j = 0; j < meses_valor.length; j++) {
				if (meses_valor[j] == 0) {

				}else{
					$("#th_"+j).text(((meses_aux[j]*100)/meses_valor[j])+"%");
				}
		...
		$.post('core/app/view/colaborador.php?parAccion=editar_capacitacion_registro', {
			...
			$.each(obj.fechas, function(index, val) {
				if(index == 0){
					...
					$("#div_add").append(`
						<div class="col-md-5 form_${index}" style="margin-top: 0.5rem;">
							<label>Mes</label>
						...
						<div class="col-md-2 form_${index}" style="text-align: center; margin-top: 0.5rem;">
							<label style="display: block;">Eliminar</label>
	...
	function actualizar(id){
		var _meses_ = $("select[name='mes[]']").map(function(){return $(this).val();}).get();
      	var _dias_ = $("input[name='dia[]']").map(function(){return $(this).val();}).get();
