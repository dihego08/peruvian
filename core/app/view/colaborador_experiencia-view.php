...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<h1><i class="glyphicon glyphicon-stats"></i> Experiencia Laboral</h1>
				...
				<div class="box-body">
					...
					<div class="row" hidden id="div_formulario" style="margin-top: 15px; background: wheat; border-radius: 4px; box-shadow: 0px 2px 2px #333; padding: 10px;">
						...
						<div class="col-md-12">
							...
							<textarea class="form-control" id="responsabilidades"></textarea>
						</div>
						<div class="col-md-4">
						...
						<div class="col-md-4">
							...
							<input type="text" id="tiempo_servicio" name="tiempo_servicio" class="form-control">
						</div>
						<div class="col-md-12">
							<label for="">Motivo del Cese</label>
							<textarea class="form-control" id="motivo_cese" style="resize: none;"></textarea>
						</div>
						<div class="col-md-12" style="text-align: center; margin-top: 15px;">
							<a class="btn btn-info" href="http://192.99.55.83/sistema/core/app/view/pdf-utiles.php?parAccion=experiencia&id_col=<?php echo $_GET['id_colaborador']; ?>">Imprimir</a>
					...
					<h5 style="font-weight: bold;">Experiencias del Colaborador</h5>
					<div class="form-row" id="div_experiencias" style="margin-top: 15px;">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if (id_colaborador != 0) {
			get_experiencia(id_colaborador);
		} else {}

		$("#id_colaborador").on("change", function() {
			get_experiencia($("#id_colaborador").val());
		});
		get_colaborador();
		$("#fecha_termino").on("change", function() {
			var day1 = new Date($("#fecha_ingreso").val());
			var day2 = new Date($("#fecha_termino").val());

			var difference = (day2.getTime() - day1.getTime()) / (1000 * 3600 * 24);

			var anio_ = difference / 360;
			var mese_ = difference / 30;
			var dias_ = difference % 30;
			$("#tiempo_servicio").val(parseInt(anio_) + " Años y " + parseInt(mese_) + " meses ");
		});
	...
	function cargar_archivo(id) {
		...
		if ($('input[name="file1"]').val() !== '') {
			if (archivo.length > 0) {
				...
				$.ajax({
					url: "core/app/view/colaborador.php?parAccion=cargar_archivo_experiencia",
					type: "POST",
					data: formData,
					dataType: "json",
					processData: false,
	...
	function get_experiencia(id) {
		...
		$.post('core/app/view/colaborador.php?parAccion=get_experiencia', {
			id: id
		}, function(data) {
			var obj = JSON.parse(data);
			$.each(obj, function(index, val) {
				$("#div_experiencias").append(`
					<div class="card col-md-4" style="padding: 0px !important; margin: 5px;">
						...
						<div class="card-body">
    						<h5 style="font-weight: bold;" class="card-title">` + val.cargo + `</h5>
    						<p class="card-text">` + val.responsabilidades + `</p>
    						<p style="font-weight: bold; margin-bottom: 0px;">` + val.fecha_ingreso + ` - ` + val.fecha_termino + `</p>
    						<p>` + val.tiempo_servicio + `</p>
							<div class="alert alert-warning">${$.trim(val.motivo_cese)}</div>
							<p><a class="" href="core/app/view/experiencia/${val.archivo}" target="_blank">${$.trim(val.archivo)}</a></p>
  						</div>
					</div>
				`);
			});
	...
	function guardar() {
		$.post('core/app/view/colaborador.php?parAccion=guardar_experiencia', {
	...
	function editar(id) {
		$.post('core/app/view/colaborador.php?parAccion=editar_experiencia', {
			id: id
		}, function(data) {
			var obj = JSON.parse(data);
			$("#empresa").val(obj.empresa);
			$("#cargo").val(obj.cargo);
			$("#responsabilidades").val(obj.responsabilidades);
			$("#fecha_ingreso").val(obj.fecha_ingreso);
			$("#fecha_termino").val(obj.fecha_termino);
			$("#tiempo_servicio").val(obj.tiempo_servicio);
			$("#motivo_cese").val(obj.motivo_cese);
	...
	function actualizar(id) {
		$.post('core/app/view/colaborador.php?parAccion=actualizar_experiencia', {
			id_colaborador: id_colaborador,
			empresa: $("#empresa").val(),
			cargo: $("#cargo").val(),
			responsabilidades: $("#responsabilidades").val(),
	...
	function eliminar(id) {
		$.post('core/app/view/colaborador.php?parAccion=eliminar_experiencia', {
			id: id
		}, function(data) {
			var obj = JSON.parse(data);
			if (obj.Result == "OK") {
