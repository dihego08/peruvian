...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <h3><i class="glyphicon glyphicon-stats"></i> VERIFICACIÓN DE COMPETENCIAS</h3>
            ...
            <div class="box">
                <div class="box-body">
                    ...
                    <hr class="w-100">
                    <h5 style="font-weight: bold;">Verificacion de Competencias del Colaborador</h5>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        if (id_colaborador != 0) {
            get_competencias(id_colaborador);
        } else {}

        $("#id_colaborador").on("change", function() {
            get_competencias($("#id_colaborador").val());
        });
        get_colaborador();
    ...
    function get_competencias(id) {
        $("#tabla_examenes").find("tbody").empty();
        $("#div_formulario").removeAttr("hidden");

        $.post('core/app/view/colaborador.php?parAccion=get_competencias', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
            $.each(obj, function(index, val) {
                ...
                $("#tabla_examenes").find("tbody").append(`
					<tr>
						...
						<td>
							<a class="" href="core/app/view/competencias/${val.archivo}" target="_blank">${$.trim(val.archivo)}</a>
						</td>
						<td>
    ...
    function cargar_archivo(id) {
        ...
        if ($('input[name="file1"]').val() !== '') {
            if (archivo.length > 0) {
                ...
                $.ajax({
                    url: "core/app/view/colaborador.php?parAccion=cargar_archivo_competencias",
                    type: "POST",
                    data: formData,
                    dataType: "json",
                    processData: false,
    ...
    function editar(id) {
        $.post('core/app/view/colaborador.php?parAccion=editar_competencias', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
    ...
    function actualizar(id) {
        $.post('core/app/view/colaborador.php?parAccion=actualizar_competencias', {
    ...
    function eliminar(id) {
        $.post('core/app/view/colaborador.php?parAccion=eliminar_competencias', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
            if (obj.Result == "OK") {
    ...
    function guardar() {
        $.post('core/app/view/colaborador.php?parAccion=guardar_competencias', {
            id_colaborador: id_colaborador,
            periodo: $("#periodo").val(),
            fecha_inicio: $("#fecha_inicio").val(),
            observaciones: $("#observaciones").val(),
