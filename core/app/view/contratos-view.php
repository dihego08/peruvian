...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <h3><i class="glyphicon glyphicon-stats"></i> CONTRATO LABORAL</h3>
                ...
                <div class="box-body">
                    ...
                    <div class="row" hidden id="div_formulario" style="margin-top: 15px; background: wheat; border-radius: 4px; box-shadow: 0px 2px 2px #333; padding: 10px;">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="date" class="form-control datepicker rounded-pill" id="fecha_inicio">
                        </div>
                        <div class="col-md-6">
                            <label>Fecha de Fin</label>
                            <input type="date" class="form-control datepicker rounded-pill" id="fecha_fin">
                        </div>
                        <div class="col-md-6">
                            <label>Tipo Contrato</label>
                            <select name="id_tipo_contrato" id="id_tipo_contrato" class="form-control rounded-pill">
                                <option value="0">--SELECCIONE--</option>
                                <option value="1">INDETERMINADO</option>
                                <option value="2">SUJETO A MODALIDAD</option>
                                <option value="3">TIEMPO PARCIAL</option>
                                <option value="4">TELETRABAJO</option>
                                <option value="5">SUSPENSIÓN DE CONTRATO</option>
                            </select>
                        </div>
                        <div class="col-md-12">
                    ...
                    <hr class="w-100">
                    <h5 style="font-weight: bold;">Contratos del Colaborador</h5>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        if (id_colaborador != 0) {
            get_contratos(id_colaborador);
        } else {}

        $("#id_colaborador").on("change", function() {
            get_contratos($("#id_colaborador").val());
        });
        get_colaborador();
    ...
    function get_contratos(id) {
        $("#tabla_examenes").find("tbody").empty();
        $("#div_formulario").removeAttr("hidden");

        $.post('core/app/view/colaborador.php?parAccion=get_contratos', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
            $.each(obj, function(index, val) {
                ...
                $("#tabla_examenes").find("tbody").append(`
					<tr>
						...
						<td>${val.fecha_fin}</td>
						<td>${val.tipo_contrato}</td>
						<td>${val.observaciones}</td>
						<td>
							<a class="" href="core/app/view/contratos/${val.archivo}" target="_blank">${$.trim(val.archivo)}</a>
						</td>
						<td>
    ...
    function cargar_archivo(id) {
        ...
        if ($('input[name="file1"]').val() !== '') {
            if (archivo.length > 0) {
                ...
                $.ajax({
                    url: "core/app/view/colaborador.php?parAccion=cargar_archivo_contrato",
                    type: "POST",
                    data: formData,
                    dataType: "json",
                    processData: false,
    ...
    function editar(id) {
        $.post('core/app/view/colaborador.php?parAccion=editar_contrato', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
            ...
            $("#id_tipo_contrato").val(obj.id_tipo_contrato);
    ...
    function actualizar(id) {
        $.post('core/app/view/colaborador.php?parAccion=actualizar_contrato', {
    ...
    function eliminar(id) {
        $.post('core/app/view/colaborador.php?parAccion=eliminar_contrato', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
            if (obj.Result == "OK") {
    ...
    function guardar() {
        $.post('core/app/view/colaborador.php?parAccion=guardar_contrato', {
            id_colaborador: id_colaborador,
            periodo: $("#periodo").val(),
            fecha_inicio: $("#fecha_inicio").val(),
            fecha_fin: $("#fecha_fin").val(),
