...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <h3><i class="glyphicon glyphicon-stats"></i> RECOMENDACIONES DE SEGURIDAD Y SALUD EN EL TRABAJO (SST)</h3>
                ...
                <div class="box-body">
                    ...
                    <div class="row" hidden id="div_formulario" style="margin-top: 15px; background: wheat; border-radius: 4px; box-shadow: 0px 2px 2px #333; padding: 10px;">
                        <div class="col-md-6">
                            ...
                            <input type="date" id="fecha_recomendacion" name="fecha_recomendacion" class="form-control datepicker rounded-pill">
                        </div>
                        <div class="col-md-6">
                            <label>Fecha de Capacitación</label>
                            <input type="date" class="form-control datepicker rounded-pill" id="fecha_capacitacion">
                        </div>
                        <div class="col-md-6">
                            <label>Tipo de Recomendación</label>
                            <input type="text" class="form-control rounded-pill" id="tipo_recomendacion">
                        </div>
                        <div class="col-md-6">
                            <label>Referencia de la recomendación</label>
                            <input type="text" class="form-control rounded-pill" id="referencia_recomendacion">
                        </div>
                        <div class="col-md-12">
                    ...
                    <hr class="w-100">
                    <h5 style="font-weight: bold;">Recomendaciones SST del Colaborador</h5>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        if (id_colaborador != 0) {
            get_recomendaciones(id_colaborador);
        } else {}

        $("#id_colaborador").on("change", function() {
            get_recomendaciones($("#id_colaborador").val());
        });
        get_colaborador();
    ...
    function get_recomendaciones(id) {
        ...
        $.post('core/app/view/colaborador.php?parAccion=get_recomendaciones', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
            $.each(obj, function(index, val) {
                ...
                $("#tabla_examenes").find("tbody").append(`
					<tr>
						<td>${val.fecha_recomendacion}</td>
						<td>${val.fecha_capacitacion}</td>
						<td>${val.tipo_recomendacion}</td>
						<td>${val.referencia_recomendacion}</td>
						<td>${val.observaciones}</td>
						<td>
							<a class="" href="core/app/view/sst/${val.archivo}" target="_blank">${$.trim(val.archivo)}</a>
						</td>
						<td>
    ...
    function cargar_archivo(id) {
        ...
        if ($('input[name="file1"]').val() !== '') {
            if (archivo.length > 0) {
                ...
                $.ajax({
                    url: "core/app/view/colaborador.php?parAccion=cargar_archivo_recomendaciones",
                    type: "POST",
                    data: formData,
                    dataType: "json",
                    processData: false,
    ...
    function editar(id) {
        $.post('core/app/view/colaborador.php?parAccion=editar_recomendaciones', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
            $("#fecha_recomendacion").val(obj.fecha_recomendacion);
            $("#fecha_capacitacion").val(obj.fecha_capacitacion);
            $("#tipo_recomendacion").val(obj.tipo_recomendacion);
            $("#referencia_recomendacion").val(obj.referencia_recomendacion);
            $("#observaciones").val(obj.observaciones);

            $("#fecha_recomendacion").focus();
    ...
    function actualizar(id) {
        $.post('core/app/view/colaborador.php?parAccion=actualizar_recomendaciones', {
            id_colaborador: id_colaborador,
            fecha_recomendacion: $("#fecha_recomendacion").val(),
            fecha_capacitacion: $("#fecha_capacitacion").val(),
            tipo_recomendacion: $("#tipo_recomendacion").val(),
            referencia_recomendacion: $("#referencia_recomendacion").val(),
    ...
    function eliminar(id) {
        $.post('core/app/view/colaborador.php?parAccion=eliminar_recomendaciones', {
            id: id
        }, function(data) {
            var obj = JSON.parse(data);
            if (obj.Result == "OK") {
    ...
    function limpiar_formulario() {
        $("#fecha_recomendacion").val("");
        $("#fecha_capacitacion").val("");
        $("#tipo_recomendacion").val("");
        $("#referencia_recomendacion").val("");
        $("#observaciones").val("");
    }

    function guardar() {
        $.post('core/app/view/colaborador.php?parAccion=guardar_recomendaciones', {
            id_colaborador: id_colaborador,
            fecha_recomendacion: $("#fecha_recomendacion").val(),
            fecha_capacitacion: $("#fecha_capacitacion").val(),
            tipo_recomendacion: $("#tipo_recomendacion").val(),
