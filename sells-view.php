...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			...
			<?php else : ?>
				<h3><i class='glyphicon glyphicon-shopping-cart'></i> VENTAS PAGOS</h3>
			<?php endif; ?>
				...
				<div class="box-body" style="margin-bottom: 2rem;">
					<fieldset>
						...
						<div class="form-group col-md-4">
							...
							<select class="form-control rounded-pill" name="tipos_documento" id="tipos_documento">
								<option value="0">SELECCIONA ...</option>
							</select>
						</div>
						<div class="form-group col-md-4">
					...
					<fieldset>
						...
						<div class="form-row">
							...
							<div class="form-group col-md-12 text-center">
								<button class="btn btn-success rounded-pill" onclick="buscar_por_fecha();"><i class="fa fa-search"></i> Filtrar</button>
							</div>
						</div>
					</fieldset>
					...
					<div>
						...
						<div class="box box-secundary" id="div_pagado" style="overflow: hidden;">
							...
							<div class="w-100 text-center">
								<button class="btn btn-success rounded-pill" id="boton_actualizar_pago">Actualizar Pago</button>
							</div>

							<h4 id="titulo_detalle">Historial de Pagos</h4>
							<div class="box box-primary">
...
<script type="text/javascript">
	...
	function detalle_venta(codigo) {
		...
		$("#titulo_detalle").append('Detalle de Venta: <a href="#' + codigo + '_">' + codigo + '</a>');
		$("#pagado").val('');
		$.get('core/app/view/venta.php', {
			...
			$.each(obj.Records, function(index, val) {
				...
				if (val.detraccion == "yes") {
					if (val.detraccion_p > 0 && val.detraccion_paga == 0) {
						...
						<span class="btn btn-sm rounded-pill btn-outline-success " onclick="guardar_pago_detraccion('${val.codigo_venta}');"><i class="fa fa-check"></i></span>`;
					}
				}

				if (val.a_cuenta > parseFloat('0.00')) {
					$("#tabla_ventas").find('tbody').append('<tr class="danger" id="' + val.codigo_venta + '_"><td>' + parseInt(index + 1) + '</td><td>' + '<a href="#" onclick="detalle_venta(\'' + val.codigo_venta + '\');" class="btn btn-sm rounded-pill btn-outline-dark"><i class="glyphicon glyphicon-eye-open"></i></a>' + '</td><td>' + val.codigo_venta + '</td><td>' + val.fecha_creacion + '</td><td>' + val.tipo_documento + '</td><td>' + val.pago + '</td><td>' + val.entrega + '</td><td>S/ ' + val.valor_pagar + '</td><td>' + val.person + '</td><td class="text-center">' + val.detraccion_p + `
						${pa_pagar}
						` + '</td><td>S/ ' + val.a_cuenta + '</td><td>' + '<a href="#" onclick="eliminar_venta(\'' + val.codigo_venta + '\');" class="btn btn-sm rounded-pill btn-outline-danger"><i class="fa fa-trash"></i></a>' + '</td>' +
						'</tr>');
				} else {
					if (pa_pagar == "") {
						$("#tabla_ventas").find('tbody').append('<tr id="' + val.codigo_venta + '_"><td>' + parseInt(index + 1) + '</td><td>' + '<a href="#" onclick="detalle_venta(\'' + val.codigo_venta + '\');" class="btn btn-sm rounded-pill btn-outline-dark"><i class="glyphicon glyphicon-eye-open"></i></a>' + '</td><td>' + val.codigo_venta + '</td><td>' + val.fecha_creacion + '</td><td>' + val.tipo_documento + '</td><td>' + val.pago + '</td><td>' + val.entrega + '</td><td>S/ ' + val.valor_pagar + '</td><td>' + val.person + '</td><td class="text-center">' + val.detraccion_p + `
						${pa_pagar}
						` + '</td><td>S/ ' + val.a_cuenta + '</td><td>' + '<a href="#" onclick="eliminar_venta(\'' + val.codigo_venta + '\');" class="btn btn-sm rounded-pill btn-outline-danger"><i class="fa fa-trash"></i></a>' + '</td>' +
							'</tr>');
					} else {
