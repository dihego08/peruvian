...
export default function PuestosView() {
  ...
  const fetchPuestos = async () => {
    try {
      const r = await api.get('/sig/puestos-crud');
      setPuestos(r.data);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.id_area) return alert('Seleccione un área');
    setSaving(true);
    try {
      await api.post('/sig/puestos-crud', { ...formData, id: editingId });
      closeModal(); fetchPuestos();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/sig/puestos-crud/${id}`);
      fetchPuestos();
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Gestión de cargos y puestos de trabajo</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading && <tr><td colSpan="4" className="px-6 py-10 text-center text-gray-400">Cargando puestos...</td></tr>}
            {!loading && puestos.length === 0 && <tr><td colSpan="4" className="px-6 py-10 text-center text-gray-400">No hay puestos registrados.</td></tr>}
            {puestos.map(p => (
              <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 text-gray-500 font-mono">#{p.id}</td>
                <td className="px-6 py-4 font-bold text-gray-800">{p.puesto}</td>
                <td className="px-6 py-4">
                  <span className="px-2.5 py-1 bg-blue-50 text-blue-700 rounded-md text-xs font-bold uppercase">
                    {p.area?.area || 'Sin Área'}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center justify-center gap-2">
                    <button onClick={() => openEdit(p)} className="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Editar">
                      <PencilSquareIcon className="h-5 w-5" />
                    </button>
                    <button onClick={() => handleDelete(p.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h2 className="text-lg font-bold text-gray-900">{editingId ? 'Editar Puesto' : 'Nuevo Puesto'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-1">
                ...
                <select
                  required
                  className="w-full p-2.5 border border-gray-300 rounded-lg focus:border-blue-500 outline-none text-sm font-medium bg-white"
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Nombre del Puesto *</label>
                <input
                  required
                  type="text"
                  ...
                  placeholder="Ej: Gerente General, Asistente..."
                />
              </div>
