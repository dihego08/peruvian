...
export default function FamiliaresModal({ colaborador, onClose }) {
  ...
  const fetchFamiliares = async () => {
    setLoading(true);
    try {
      const r = await api.get(`/sig/colaboradores/${colaborador.id}/familiares`);
      setFamiliares(r.data);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('¿Eliminar familiar?')) return;
    try {
      await api.delete(`/sig/colaboradores/${colaborador.id}/familiares/${id}`);
      fetchFamiliares();
    } catch (e) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/sig/colaboradores/${colaborador.id}/familiares/${formData.id}`, formData);
      } else {
        await api.post(`/sig/colaboradores/${colaborador.id}/familiares`, formData);
      }
      setFormData(EMPTY);
      setIsEditing(false);
      fetchFamiliares();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50 rounded-t-2xl">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Familiares</h2>
        ...
        <div className="flex-1 overflow-auto p-6 flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-1/3 bg-gray-50 p-4 rounded-xl border border-gray-100 shrink-0">
            <h3 className="font-bold text-gray-700 mb-4">{isEditing ? 'Editar Familiar' : 'Nuevo Familiar'}</h3>
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className={labelClasses}>DNI *</label>
                <input required type="text" className={inputClasses} value={formData.dni} onChange={e => setFormData({ ...formData, dni: e.target.value })} />
              </div>
              <div>
                <label className={labelClasses}>Nombres *</label>
                <input required type="text" className={inputClasses} value={formData.nombre} onChange={e => setFormData({ ...formData, nombre: e.target.value })} />
              </div>
              <div>
                <label className={labelClasses}>Apellidos *</label>
                <input required type="text" className={inputClasses} value={formData.apellidos} onChange={e => setFormData({ ...formData, apellidos: e.target.value })} />
              </div>
              <div>
                <label className={labelClasses}>Parentesco *</label>
                <input required type="text" className={inputClasses} value={formData.parentesco} onChange={e => setFormData({ ...formData, parentesco: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                <label className={labelClasses}>Lugar Nacimiento</label>
              ...
              <div>
                ...
                <input type="text" className={inputClasses} value={formData.telefono} onChange={e => setFormData({ ...formData, telefono: e.target.value })} />
              </div>
          ...
          <div className="w-full lg:w-2/3">
            {loading ? (
              ...
              <div className="text-center py-10 text-gray-500 bg-gray-50 rounded-xl border border-dashed border-gray-200">No hay familiares registrados.</div>
            ) : (
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {familiares.map(f => (
                      <tr key={f.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">{f.dni}</td>
                        <td className="px-4 py-3 font-medium">{f.nombre} {f.apellidos}</td>
                        <td className="px-4 py-3">{f.parentesco}</td>
                        <td className="px-4 py-3">{f.telefono}</td>
                        <td className="px-4 py-3">
                          <div className="flex justify-center gap-2">
                            <button onClick={() => handleEdit(f)} className="text-blue-600 hover:text-blue-800 p-1 bg-blue-50 rounded-md cursor-pointer">
