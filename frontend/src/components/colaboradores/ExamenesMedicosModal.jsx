...
export default function ExamenesMedicosModal({ colaborador, onClose }) {
  ...
  const fetchExamenes = async () => {
    setLoading(true);
    try {
      const r = await api.get(`/sig/colaboradores/${colaborador.id}/examenes_medicos`);
      setExamenes(r.data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleEdit = (e) => {
    setFormData({ ...e, archivo: null });
    setIsEditing(true);
  };

  const handleDelete = async (id) => {
    if (!window.confirm('¿Eliminar examen médico?')) return;
    try {
      await api.delete(`/sig/colaboradores/${colaborador.id}/examenes_medicos/${id}`);
      fetchExamenes();
    } catch (e) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.post(`/sig/colaboradores/${colaborador.id}/examenes_medicos/${formData.id}`, payload, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        await api.post(`/sig/colaboradores/${colaborador.id}/examenes_medicos`, payload, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
      setFormData(EMPTY);
  ...
  const getFileUrl = (filename) => {
    ...
    return `${import.meta.env.VITE_API_URL?.replace('/api', '') || ''}/storage/examenes_medicos/${filename}`;
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50 rounded-t-2xl">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Certificados Médicos</h2>
        ...
        <div className="flex-1 overflow-auto p-6 flex flex-col lg:flex-row gap-6">
          <div className="w-full lg:w-1/3 bg-gray-50 p-4 rounded-xl border border-gray-100 shrink-0">
            <h3 className="font-bold text-gray-700 mb-4">{isEditing ? 'Editar Certificado' : 'Nuevo Certificado'}</h3>
          ...
          <div className="w-full lg:w-2/3">
            {loading ? (
              ...
              <div className="text-center py-10 text-gray-500 bg-gray-50 rounded-xl border border-dashed border-gray-200">No hay certificados médicos registrados.</div>
            ) : (
                    ...
                    {examenes.map(c => (
                      <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center">
                          {c.archivo ? (
                            <a href="#" onClick={(e) => handleDocumentClick(e, c.archivo, 'examenes_medicos')} className="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 text-xs bg-blue-50 px-2 py-1 rounded-md"><DocumentArrowDownIcon className="w-4 h-4" /> Ver</a>
